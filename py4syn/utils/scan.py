...
def plot():
    ...
    global YFIELD
    global FIT_SCAN
    p = Plotter("Plot", xlabel=XFIELD, ylabel=YFIELD, line_style="-", line_marker=".", line_color="blue", daemon=True)
    x = SCAN_DATA[XFIELD]
    y = SCAN_DATA[YFIELD]    
    p.plot(x, y)
        ...
        if(waitingTime):
            ...
            if(isinstance(countTime, collections.Iterable)):
                if(s.getNumberOfPoints() != len(countTime)):
                    raise Exception("Error creating the scan. The time array ("+str(len(countTime))+" points) and the scan ("+str(s.getNumberOfPoints())+" points) must have the same number of points. Please check.")
        ...
        if(waitingDelay):
            ...
            if(isinstance(delayTime, collections.Iterable)):
                if(s.getNumberOfPoints() != len(delayTime)):
                    raise Exception("Error creating the scan. The delay array ("+str(len(delayTime))+" points) and the scan ("+str(s.getNumberOfPoints())+" points) must have the same number of points. Please check.")
...
def mesh(*kwargs):
    ...
    for item in kwargs:
        ...
        if(waitingTime):
            ...
            if(isinstance(countTime, collections.Iterable)):
                if(s.getNumberOfPoints() != len(countTime)):
                    raise Exception("Error creating the mesh. The time array ("+str(len(countTime))+" points) and the mesh ("+str(s.getNumberOfPoints())+" points) must the same number of points. Please check.")
        ...
        if(waitingDelay):
            ...
            if(isinstance(delayTime, collections.Iterable)):
                if(s.getNumberOfPoints() != len(delayTime)):
                    raise Exception("Error creating the mesh. The delay array ("+str(len(delayTime))+" points) and the mesh ("+str(s.getNumberOfPoints())+" points) must have the same number of points. Please check.")
...
class Scan(object):
    ...
    def addScanParam(self, param):
        if(self.__scanType == ScanType.SCAN):
            if (len(self.__scanParams) != 0) and (param.getNumberOfPoints() != self.__minNumberOfPoints):
                raise Exception("All devices must have the same number of points, please check.")
            else:
    ...
    def start(self):
        ...
        try:
            ...
            if(self.__scanType == ScanType.SCAN):            
                ...
                self.__startTimestamp = self.__endTimestamp = datetime.datetime.now()
            ...
            if(FILENAME != None and FILENAME != "" and not PARTIAL_WRITE and FILE_WRITER != None):
                    print("\tSaving data to file")
                    FILE_WRITER.setEndDate(self.__endTimestamp)
                    FILE_WRITER.writeHeader()
                    FILE_WRITER.writeData(idx=-1)
    ...
    def __printAndPlot(self, **kwargs):
        p = self.getPlotter()    
        if(PLOT_GRAPH):
            p.plot(SCAN_DATA[XFIELD][-1], SCAN_DATA[YFIELD][-1], axis=self.getPlotterAxis())
    ...
    def __initialize(self):
        ...
        if(PLOT_GRAPH):
            if(SCAN_PLOTTER == None):
                p = Plotter("Scan: " + str(FILENAME), daemon=DAEMON_GRAPH)
                p.createAxis("", label=YFIELD, xlabel=XFIELD, ylabel=YFIELD, grid=True, line_style="-", line_marker=".", line_color="blue")
                SCAN_PLOTTER = p
            else:
                p = SCAN_PLOTTER
            ...
            if(SCAN_PLOTTER_AXIS == None or SCAN_PLOTTER_AXIS < 1):
                SCAN_PLOTTER_AXIS = 1
    ...
    def doMesh(self):
        ...
        for pointIdx in range(0, self.getNumberOfPoints()):
            ...
            for deviceIdx in range(0, self.getNumberOfParams()):
                ...
                idx = calculatePositionIndex(pointIdx, multiplicationIndexArray[deviceIdx], param.getNumberOfPoints())
...
class ScanParam(object):
    def __init__(self, device):
        ...
        #    raise Exception("Error: Only scannable devices can be used on scans. Please Check.")
    ...
    def setPoints(self, start, end=None, steps=None):
        ...
        if(isinstance(d, PseudoMotor)):
            if(not d.canPerformMovement(min(p))[0]):
                raise Exception("Error. The lower value exceeds the Low Limit Value for this device. Device: "+str(d))
            
            if(not d.canPerformMovement(max(p))[0]):
                raise Exception("Error. The lower value exceeds the High Limit Value for this device. Device: "+str(d))
        else:
            ...
            if(ll > min(p)):
                raise Exception("Error. The lower value exceeds the Low Limit Value for this device. Device: "+str(d)+", Low Limit: "+str(ll))

            if(hl < max(p)):
                raise Exception("Error. The higher value exceeds the High Limit Value for this device. Device: "+str(d)+", High Limit: "+str(hl))
...
def setPreScanCallback(c):
    ...
    global PRE_SCAN_CALLBACK
    if not callable(c):
        raise Exception("Error. Only functions can be used as callbacks. Please check.")
    PRE_SCAN_CALLBACK = c
...
def setFileWriter(writer):
    ...
    if not isinstance(writer, FileWriter):
        raise Exception("Error. The parameter is not a valid File Writer. Please check.")
...
def setScanPlotter(p):
    ...
    if isinstance(p, Plotter):
        ...
        raise Exception("Error. Parameter is not a valid Plotter. Please check.")
...
def setScanPlotterAxis(ax):
    ...
    ----------
    >>> p = Plotter("My Plotter ", daemon=False)
    >>> p.createAxis("", label="Data label", xlabel="Energy", ylabel="I0", grid=True, line_style="-", line_marker=".", line_color="blue")
    >>> setScanPlotter(p)
