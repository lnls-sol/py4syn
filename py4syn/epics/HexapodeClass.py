...
class Hexapode(IScannable,StandardDevice):
    ...
    def onStatusChange(self,value,**kw):
        """
        Returns True if the target position was reachedy
        """
        if ( abs(float(value) - float(self.pos)) >1e-4):
            self.moving=True
        else:
            self.moving=False
    ...
    def setAbsolutePosition(self, pos, waitComplete=True):
        ...
        self.fim1=time.time()-self.inicio1
        print("terminou: ",self.fim1)
        
        self.inicio1=time.time()
        self.pos=pos
        
        self.hexapode.put('STATE#PANEL:SET',11)
        self.hexapode.put('MOVE#PARAM:'+self.axis, self.pos)

        #ret, msg = self.canPerformMovement(self.pos)
        #if(not ret):
        #    raise Exception("Can't move hexapod axis "+self.axis+" to desired position: "+str(self.pos)+ ", " + msg)
    ...
    def getLowLimitValue(self):
        return -20
        #self.getLimits(0,self.axis_number)[0]

    def getHighLimitValue(self):
        return 20
        #self.getLimits(0,self.axis_number)[1]
...
#------------------------------------------------------------------------------------------
    ...
    def getLowLimitValue(self):
           ...
           elif(self.axis_number < 0 or self.axis_number > 6):
                raise ValueError("Invalid value for axis argument. It should be between 1 and 6")
